...
export default function AdminProducts() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const data = await api.get("/api/products/perfumes/");
      setProducts(data);
    } catch {
      toast.error("Failed to load products");
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/products/perfumes/${String(id)}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingProduct) {
        await api.put(`/api/products/perfumes/${String(editingProduct.id)}/`, productData);
        ...
        const newProduct = await api.post("/api/products/perfumes/", {
          ...productData,
          id: Date.now().toString()
        });
        setProducts([...products, newProduct]);
            ...
            <div className="space-y-4">
              ...
              {formData.image && (
                <img 
                  ...
                  onError={(e) => {
                    e.target.src = "https://placehold.co/400x200/1e1e1e/ffffff?text=Invalid+URL";
                  }}
                />
              )}
            </div>
