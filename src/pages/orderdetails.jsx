...
//   const [order, setOrder] = useState(location.state?.oredr || null);
//   const [loading, setLoading] = useState(! location.state ?.order);

//   useEffect(() => {
//     if (!location.state?.order) {
//       fetchOrderDetails(); // only fetch if no state passed
//     }
//   }, [id]);
...
//                     Placed on {new Date(order.orderDate || order.created_at).toLocaleDateString('en-IN', {
//                       weekday: 'long',
//                       year: 'numeric',
//                       month: 'long',
//                       day: 'numeric'
...
//                   <p className="font-medium">{order.address || "Not specified"}</p>
//                 </div>
//                 {order.phone && (
//                   <div>
...
function OrderDetails() {
  ...
  const fetchOrderDetails = async () => {
    ...
    try {
      ...
      // ✅ First try location state (came from orders page)
      if (location.state?.order) {
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="text-center">
            <p className="text-lg mb-4">Order not found</p>
            <button onClick={() => navigate("/order")} className="bg-black text-white px-6 py-2 rounded">
              Back to Orders
            </button>
          </div>
        </div>
            ...
            <div className="border rounded-lg p-6">
              ...
              <div className="space-y-4">
                {order.items?.map((item, index) => (
                  <div key={index} className="flex items-center gap-4 border-b pb-4 last:border-0">
                    <img
                      src={item.perfume_image || "https://placehold.co/100x100"}
